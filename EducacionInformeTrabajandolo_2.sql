...
select 
	...
	sum (r.SEG_Estudian_MA_SCV) as SEG_Estudian_MA_SCV,
	sum( r.SEG_Graduados ) as SEG_Graduados,
	sum (r.SEG_Estudian_MA_CV+r.SEG_Estudian_MA_SCV+r.SEG_Graduados) as SEG_Estudian,
	sum (r.SEG_Estudian_MA_CV+r.SEG_Estudian_MA_SCV+r.SEG_Graduados+ r.PE_Graduados +r.PE_Estudian_MA_CC) as Estudian_FinalizarPeriodo,
	( sum(r.SEG_Estudian_MA_CV+r.SEG_Estudian_MA_SCV+r.SEG_Graduados+ r.PE_Graduados +r.PE_Estudian_MA_CC)*1.0/COUNT(r.Id_Declaracion))
	    as Porc_AccesoEducacion,
    COUNT(r.Id_Declaracion)- sum(r.SEG_Estudian_MA_CV+r.SEG_Estudian_MA_SCV+r.SEG_Graduados+ r.PE_Graduados +r.PE_Estudian_MA_CC) as PendientesPorAcceso
from
(
select 
    q.*,    
	...
	case when 
		  ...
		  then 1 else 0 end   as SEG_Estudian_MA_SCV,
	case when 
	      q.SEG_Remitir=1
		  and
		  (
...
group by r.Regional, r.MesAtencion
order by r.Regional, r.MesAtencion
