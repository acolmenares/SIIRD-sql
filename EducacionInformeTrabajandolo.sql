...
select
	...
	sum(p.Estudia_PEntregaMM) as Estudia_PEntregaMM,
...
case when (E_PE.Descripcion='Si' and MUN_PE.Descripcion =Lentrega.Descripcion  ) then 1 else 0 end as Estudia_PEntregaMM,
case when (E_PE.Descripcion='Si' and MUN_PE.Descripcion <>Lentrega.Descripcion  ) then 1 else 0 end as Estudia_PEntregaOMun,
case when not (E_PE.Descripcion='Si' and  MUN_PE.Descripcion =Lentrega.Descripcion) and not(isnull(Personas.Id_Motivo_NoEstudio,0)=@id_yasegraduo )  then 1 else 0 end as NoEstudia_PEntrega,
...
-- '--' as NE_PEntrega,
case when  
not ((E_PE.Descripcion='Si' and  MUN_PE.Descripcion =Lentrega.Descripcion) and not(isnull(Personas.Id_Motivo_NoEstudio,0)=@id_yasegraduo ))  -- NoEstudiaPEntrega
and 
(
 ( 
   E_SE.Descripcion='Si' and C_SE.Descripcion='Si' and MUN_SE.Descripcion=Lentrega.Descripcion  -- estudia con certificado mm en SE
   and not
   (isnull(E_SEG.Descripcion,'')='Si' and (C_SEG.Descripcion='Si' or e_seguimiento.Verificado_Entidad=1) and isnull(MUN_SEG.Descripcion,'')=Lentrega.Descripcion)
 )
 or
 ( 
   not (isnull(E_SE.Descripcion,'No')='Si' and isnull(C_SE.Descripcion,'No')='Si' and isnull(MUN_SE.Descripcion,'')=Lentrega.Descripcion)
   and
   (E_SEG.Descripcion='Si' and (C_SEG.Descripcion='Si' or e_seguimiento.Verificado_Entidad=1) and MUN_SEG.Descripcion=Lentrega.Descripcion)  --estudia con certificado/Verificado mm en SEG
 )
)
